<template>
  ...
  >
    ...
    <template slot="templateAutocomplete" slot-scope="{ value }">
      ...
      <q-item-tile sublabel>{{value.object.matricula}}</q-item-tile>
    </template>
    <template slot="modal">
      <modal-search-funcionario
        :searchMethod="this.searchMethod"
        ref="modal"
        :label="label"
        @onSelect="select"
...
import ModalSearchFuncionario from '../modal/ModalSearchFuncionario'
...
export default {
  ...
  methods: {
    ...
    changeMatricula () {
      this.$refs.searchInput.changeInput(this.filtro.matricula)
    },
    clean () {
      ...
      this.model = {}
      this.$nextTick(() => this.$refs.inputMatricula.focus())
    },
    openModal () {
      this.$refs.modal.open()
    },
    select (funcionario) {
      this.selecionado = `${funcionario.matricula} - ${funcionario.nome}`
